...
def start(anonyList, highScore, dataSelect, process):
    ...
    orig = pd.read_csv(f'2exchange-{dataSelect}/{dataSelect}.csv', header=None)
    df = orig.copy()
    with open(f'2exchange-{dataSelect}/log.txt', 'r') as file:
        log = file.read()+'\n'
